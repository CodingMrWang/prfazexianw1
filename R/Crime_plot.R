...
#' CrimePlot(crime_data, offence_description, suburbs)
 CrimePlot <- function(crime_data, offence_description, suburbs) {
  ...
  print(1)
  plot_data <- crime_data[(crime_data$suburb %in% suburbs & crime_data$offence_level_3 %in% offence_description),list(suburb = crime_data$suburb, total_offence_count = sum(offence_count)), keyby = date]
  print(2)
